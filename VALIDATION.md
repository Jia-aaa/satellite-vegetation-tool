...
**📍 任务一第 5 节 5 条验收标准 → 章节速查**

| 任务验收 | 看哪一节 | 自检脚本 |
|---|---|---|
| 1. 手算像素 NDVI 对拍 | [§5 手算验证](#5-手算验证) | `checks/manual_pixel_check.py` |
| 2. NDVI ∈ [-1, 1] | [§6 范围检查](#6-范围检查) | `checks/validate_ndvi.py` + `checks/check_outputs.py` |
| 3. nodata 不参与计算 | [§4 自动验收检查](#4-自动验收检查) + [§8 边界情况说明](#8-边界情况说明) | `checks/check_outputs.py` |
| 4. 输出与输入地理对齐 | [§7 地理参考检查](#7-地理参考检查) | `checks/check_outputs.py` + `checks/validate_ndvi.py` |
| 5. QGIS 叠图合理 | [§9 QGIS 叠图检查](#9-qgis-叠图检查) | 人工 |
